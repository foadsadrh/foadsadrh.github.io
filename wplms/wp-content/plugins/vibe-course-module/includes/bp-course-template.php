			...
			foreach($course_items as $key => $item){
				if(is_numeric($item)){
					...
					$curriculum[] = array(
						...
						'link'		=>	(( vibe_validate($free_access) || ($post->post_author == get_current_user_id()) || current_user_can('manage_options') || $curriculum_course_link)?get_permalink($item).'?id='.$course_id:''),	
						'duration' 	=>  $duration,
...
function bp_course_quiz_results($quiz_id,$user_id,$course=NULL){
	...
	echo '<div id="total_marks">'.__('Total Marks','vibe').' '.($quiz_status?'<strong><span>'.$sum.'</span>'.(empty($total_sum)?'':' / '.$total_sum).'</strong>':'<strong><i class="icon-alarm"></i></strong>').' </div></div>';
}
