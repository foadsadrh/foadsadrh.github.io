...
class EVO_generator {
	...
	// GENERATE TIME for event
		public function generate_time_(
			...
			// Include day name
				if( isset($DATE_end_val['D'])) $data_array['end']['day'] = $DATE_end_val['D'];
		...
		public function generate_event_data(
			...
			foreach($event_list_array as $event_):
				...
				// set how a single event would interact
					$event_ux_val = $EVO_Event->get_prop('_evcal_exlink_option')? $EVO_Event->get_prop('_evcal_exlink_option'):1;
				...
				// year/month long or not
					$__year_long_event = $EVO_Event->is_year_long();
					$__month_long_event = $__year_long_event? false: ( $EVO_Event->is_month_long()? true:0);
				...
				// if this event featured
					$_eventInClasses['__featured'] = 	$EVO_Event->is_featured();
					$_eventInClasses['_completed'] = 	$EVO_Event->is_completed();
					$_eventInClasses['_cancel'] = 		$EVO_Event->is_cancelled();
				...
				// All day event variables
					$_is_allday = 		$EVO_Event->is_all_day();
					$_hide_endtime = 	$EVO_Event->is_hide_endtime();
				...
				*/
				...
				$_event_date_HTML = $this->generate_time_($DATE_start_val, $DATE_end_val, $ev_vals, $evcal_lang_allday, $focus_month_beg_range, $FOCUS_month_int, $event_start_unix, $event_end_unix, $cal_hide_end_time);
				...
				// EACH DATA FIELD
					// EVENT FEATURES IMAGE
						...
						if($img_id!=''){
							...
							$img_med_src = wp_get_attachment_image_src($img_id, apply_filters('evo_event_medium_image_size','medium'));
							$img_thumb_src = wp_get_attachment_image_src($img_id, apply_filters('evo_event_thumbnail_image_size','thumbnail'));
				...
				/** CONSTRUCT the EVENT CARD	 **/
					...
					if($_event_card_on && !empty($_eventcard) && count($_eventcard)>0){
						...
						echo  eventon_eventcard_print($_eventcard, $EVO_Event, $this->evopt1, $this->evopt2);
				...
				// EVENT TOP
					...
					// close eventtop
							...
							$__eventtop = apply_filters('eventon_eventtop_one', $__eventtop, $ev_vals, $_passVal, $EVO_Event);
						$__eventtop['close1'] = array();
							$__eventtop = apply_filters('eventon_eventtop_two', $__eventtop, $ev_vals, $_passVal, $EVO_Event);
					...
					if(!empty($__eventtop) && count($__eventtop)>0){
						...
						echo  eventon_get_eventtop_print($__eventtop, $EVO_Event, $this->evopt1, $this->evopt2);
				...
				// SCHEME SEO
					...
					if(!$show_schema){						
						...
						foreach(apply_filters('evo_event_schema',array(
							...
							'startDate'=>array(
								'type'=>'meta',
								'content'=> $DATE_start_val['Y'].'-'.$DATE_start_val['n'].'-'.$DATE_start_val['j']. ( !$EVO_Event->is_all_day()? 'T'.$DATE_start_val['H'].':'.$DATE_start_val['i']:'')
							),
							'endDate'=>array(
								'type'=>'meta',
								'content'=> $DATE_end_val['Y'].'-'.$DATE_end_val['n'].'-'.$DATE_end_val['j']. ( !$EVO_Event->is_all_day()? 'T'.$DATE_end_val['H'].':'.$DATE_end_val['i']:'')
							),
							'eventStatus'=>array(
								'type'=>'meta',
